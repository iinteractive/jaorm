...
class ResultSet {
  ...
  _new_self() {
    ...
    const copy = self._schema.rs(self._table);
    copy._process_arguments(copy, self._attributes, self._driver_options);
    return copy;
  }
  ...
  async all(attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    if (
      (attributes !== undefined || driver_options !== undefined) &&
      redispatch !== false
    ) {
      const new_rs = self._new_self();
      new_rs._process_arguments(new_rs, attributes, driver_options);
      return new_rs.all({}, {}, false);
    }
  ...
  check_redispatch(attributes, driver_options, redispatch) {
    const self = this;
  ...
  async count(attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(attributes, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.count({}, {}, false);
    }

    return await self._schema.driver.count(self.driver_arguments());
  }

  async first(attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(attributes, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.first({}, {}, false);
    }
  ...
  async one(attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(attributes, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.one({}, {}, false);
    }
  ...
  async only_one(attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(attributes, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.only_one({}, {}, false);
    }
  ...
  async none(attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(attributes, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.none({}, {}, false);
    }
  ...
  // WRITE OPERATIONS
  async create(attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(attributes, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.create({}, {}, false);
    }
  ...
  async update_all(new_attributes, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(undefined, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.update_all(new_attributes, {}, false);
    }
  ...
  async destroy_all(attributes = {}, driver_options, redispatch) {
    const self = this;

    // Re-dispatch unless we already have, so we don't pollute the original resultset
    const rd_rs = self.check_redispatch(attributes, driver_options, redispatch);
    if (rd_rs) {
      return rd_rs.destroy_all({}, {}, false);
    }

    const options = self.driver_arguments();
    options.attributes = self._attributes;
